...
export function ChallengeItem({ data }: ChallengeItemProps) {
  return (
    ...
    >
      <div className="relative h-[220px]  w-full object-fill rounded-t-lg overflow-hidden">
        <Image
          fill
          alt={`challenge ${data.title} preview`}
          src={data.preview}
          className=" group-hover:scale-105 block h-full absolute inset-0 transition-transform w-full"
        />
      </div>
      ...
      <p className="max-w-[336px] text-sm  text-slate-400 line-clamp-2	mt-2">
        {data.description}
      </p>
    </Link>
  );
