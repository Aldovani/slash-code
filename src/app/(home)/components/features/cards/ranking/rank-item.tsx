...
export function RankItem({ name, position, score }: RankItemProps) {
  return (
    <li className="px-4 py-3 gap-8 flex justify-between items-center border border-slate-200 rounded-lg">
      ...
      {position > 3 && <span className="text-slate-500">{position}</span>}
      <span className="text-slate-500 font-medium truncate text-[8px] sm:text-base	">
        {name}
      </span>

      <span className="sm:block hidden text-slate-500 font-medium text-xs sm:text-base">
        {score}
      </span>
    </li>
  );
